...
static bool handle_stolen_access(struct kvm_vcpu *vcpu, gpa_t gpa, u64 error_code)
{
	...
	/*
	 * If we don't have a valid GVA in some situations like the guest
	 * attempting to load guest PDPTEs to a stolen GPA bit, then just
	 * inject a GP.
	 */
...
static int handle_ept_violation(struct kvm_vcpu *vcpu)
{
	...
	error_code |= stolen_to_pf_error(vcpu->kvm, gpa);
...
static void vmx_flush_pml_buffer(struct kvm_vcpu *vcpu)
{
	...
	for (; pml_idx < PML_ENTITY_NUM; pml_idx++) {
		...
		gpa = pml_buf[pml_idx] & ~gpa_stolen_mask(vcpu->kvm);
...
static __init int hardware_setup(void)
{
	...
	kvm_has_tdp_exec_only = enable_ept && cpu_has_vmx_ept_execute_only();
