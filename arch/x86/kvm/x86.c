...
{
	return rsvd_bits(cpuid_maxphyaddr(vcpu), 63) | rsvd_for_page_tables(vcpu->kvm) |
...
int load_pdptrs(struct kvm_vcpu *vcpu, struct kvm_mmu *mmu, unsigned long cr3)
{
	...
	ret = kvm_read_guest_page_mmu(vcpu, mmu, pdpt_gfn, pdpte,
				      ...
				      PFERR_READ_MASK|PFERR_WRITE_MASK);
	if (ret < 0) {
		ret = 0;
		goto out;
	}
...
split_irqchip_unlock:
	...
	case KVM_CAP_EXEC_ONLY:
		...
		if (cap->args[0] >= boot_cpu_data.x86_phys_bits || cap->args[0] <= 36 ||
		    !kvm_has_tdp_exec_only || kvm->arch.tdp_mmu_enabled)
			r = -EINVAL;
		else
...
{
	u32 access = ((kvm_x86_ops.get_cpl(vcpu) == 3) ? PFERR_USER_MASK : 0)
		| (write ? PFERR_WRITE_MASK : PFERR_READ_MASK);
...
int kvm_arch_init(void *opaque)
{
	...
	kvm_mmu_set_mask_ptes(PT_USER_MASK, PT_ACCESSED_MASK,
			...
			PT_PRESENT_MASK, 0, sme_me_mask, 0);
	kvm_timer_init();
