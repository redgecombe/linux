...
static void module_map(const struct module *mod, bool after_init)
{
	...
	if (!after_init) {
		...
		perm_writable_finish(mod->core_layout.ro_after_init.alloc);
	}
...
bool __weak module_perm_alloc(struct module_layout *layout)
{
	...
	if (IS_ENABLED(CONFIG_MMU))
		...
		area = kmalloc(sizeof(struct vm_struct), flags);
		if (!area) {
			module_memfree(addr);
			return NULL;
		}
	 ...
	 * module_perm_alloc().
	 */
	layout->text.alloc = kmalloc(sizeof(struct perm_allocation), flags);
	layout->ro.alloc = kmalloc(sizeof(struct perm_allocation), flags);
	layout->ro_after_init.alloc = kmalloc(sizeof(struct perm_allocation), flags);
	layout->rw.alloc = kmalloc(sizeof(struct perm_allocation), flags);
