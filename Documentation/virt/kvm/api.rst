...
When XO is enabled the XO bit cannot be set in any variable-range MTRR MSRs. If
KVM XO enabled and the MTRR MSRs are written with the XO bit set in the mask or
base, the vcpu will receive a #GP. Variable range MTRRs covering the xo bit = 0
range will apply to both aliases. If an variable range MTRR is set with a range
that includes xo alias gpas before XO is enabled, it will not apply to XO
alias gpas if XO is later enabled.
